...
Diese Seite ist die persönliche Webpräsenz von **Michael** — Data Scientist und Schöpfer autonomer kognitiver Systeme. Der Untertitel lautet: *Kausale Inferenz. Metaphysische Souveränität.*

Die Seite beschreibt Michaels Arbeit an Infrastruktur zur Entropie-Abwehr und Wahrung ontologischer Unabhängigkeit.

## Seitenstruktur

### 1. Hero (Startbereich)
- Name: **MICHAEL**
- Tagline: Kausale Inferenz. Metaphysische Souveränität.
- Beschreibung: Data Scientist und Schöpfer autonomer kognitiver Systeme. Konstruktion von Infrastruktur zur Entropie-Abwehr und Wahrung ontologischer Unabhängigkeit.
- Visuell unterlegt mit einer interaktiven Partikel-Netzwerk-Animation.

### 2. Exocortex – Infrastruktur der Souveränität
...
| Säule | Beschreibung |
|---|---|
| **Orchestrierung** | Home Assistant (Aktionsschicht) gekoppelt mit eigenem Orchestrator (Entscheidungsschicht). |
| **Memory & Inference** | RDF-Store als Single Source of Truth. Lokale LLMs via Bare-Metal Proxmox Cluster (AI X1 Pro). |
| **Operationen** | SecDevOps, GitOps, PyWhy für kausale Inferenz-Modellierung. |
...
| Gnade | Radikale Akzeptanz des IST-Zustands. |
...
| Axiom | Bedeutung |
|---|---|
| Signal-Diskriminierung | Energie-Meisterschaft. Trennung von Impuls (Rauschen) und Intuition (Signal). |
| Hierarchische Kontrolle | Meisterschaft der Ebenen. Subordination des Niederen Geistes unter den Höheren Geist. |
| Gesetzes-Konformität | Göttliche Gesetze. Operation via Intent, Consent und Authority im Einklang mit dem Naturrecht. |
| Vektor-Ausrichtung | Klärung der Motivation. Shift von "Service-to-Self" (Entropie) zu "Service-to-Others" (Negentropie). |
| Homöostase | Energetische Balance. Aktive Wiederherstellung von Gleichgewicht und Harmlosigkeit. |
| Interne Referenzierung | Innere Führung. Autonome Steuerung; Immunität gegen externe Manipulation. |
| Ressourcen-Logik | Loslassen von Besitz. Transition zu Stewardship (Verwaltung von Energiefluss). |
| Angewandte Kognition | Anwendung von Wissen. Weisheit wird nur durch Tat (Dienst) validiert. |
| Mustererkennung | Meisterung der Täuschung. Durchdringung der holografischen Täuschung und Ego-Filter. |
...
### 4. Nexus – Protokolle
Asynchrone, verschlüsselte und dezentrale Koordination. Enthält Links zu:
- **ChatOps** – Chat-basierte Operationen
- **Signatur** – Kryptographische Identität
- **Föderation** – Dezentrale Vernetzung
- **Physischer Node** – Physischer Standort / Hardware

## Schlüsselbegriffe

- **Exocortex**: Ein externes kognitives System — eine Erweiterung des menschlichen Geistes durch autonome, selbst gehostete Infrastruktur.
...
- **Kausale Inferenz**: Statistische und algorithmische Methoden zur Identifikation von Ursache-Wirkungs-Beziehungen (z.B. mit PyWhy/DoWhy).
...
- **Animierte Partikel**: Die Hero-Sektion zeigt ein interaktives 3D-Partikelnetzwerk, das auf Mausbewegungen reagiert.
...
5. Wenn nach technischen Details gefragt wird (Proxmox, RDF, PyWhy, GitOps), gib kompetente Antworten.
