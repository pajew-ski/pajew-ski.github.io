...
5. **Relationships** (`#relationships`, heading "Beziehungen"): relationship anarchy, 6 principle cards + 12 Building Block cards (both reuse `PrincipleCard`; `blocks` = `{title, desc}`, one-line definitions matching the card compression and the llms files)
