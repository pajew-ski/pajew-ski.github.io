...
import { AxiomCard, type AxiomItemData } from './Axioms';
...
export function Relating() {
  const { t } = useTranslation();

  const principles = t('relating.principles', { returnObjects: true }) as AxiomItemData[];
  const dimensions = t('relating.dimensions', { returnObjects: true }) as DimensionGroup[];

  return (
    <section
      className="py-32 bg-neutral-100 dark:bg-neutral-900 text-foreground transition-colors duration-500 overflow-hidden"
      aria-labelledby="relating-heading"
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {principles.map((item, index) => (
            <AxiomCard key={index} item={item} index={index} />
          ))}
        </div>
